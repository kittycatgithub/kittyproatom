...
const Cart = () => {
    ...
    <div className=" md:flex-row py-16 max-w-6xl w-full px-3 mx-auto">
      ...
      <div className="grid lg:grid-cols-2 justify-between max-w-6xl space-y-3">
        ...
        {cart.map((product, index) => (
          <div
            key={index}
            className="grid text-gray-700 border-1 border-gray-300 min-w-[340px]  md:max-w-lg items-center text-sm md:text-base font-normal"
          >
  ...
  : null
                ...
                {product.category || product.keyword === "catering"
                  ? Object.entries(
                      product.name === "Short Menu" ||
                        product.name === "Breakfast"
                        ? Object.fromEntries(
                            Object.entries(product.selectedOptions).filter(
                              ([_, value]) => value !== null,
                            ),
                          )
                        : product.selectedOptions,
                      ...
                      if (Array.isArray(value)) {
                        ...
                        displayValue =
                          arrValues.length > 0 ? arrValues.join(", ") : "null";
                      } else if (typeof value === "string") {
                      ...
                      return (
                        <p
                          key={key}
                          className="justify-around text-justify content-between"
                        >
                ...
                {(product.category === "meal-thali-snack-boxes" ||
                  product.keyword === "meal-thali-snack-boxes") &&
                product.name !== "Snack Box A" &&
                product.name !== "Snack Box B"
                  ? product.menu &&
  ...
  Object.entries(product.selectedOptions).map(([key, value], index) => {
    ...
    return null;
                  ...
                  ? product.productDetails &&
                    Object.entries(product.productDetails).map(
                      ([key, value], index) => {
                        ...
                        return (
                          <p key={index} className="text-sm">
                            {categoryName} ⟶ {itemName} — {value.qty}{" "}
                            {value.unit}
                          </p>
                        );
                      },
            ...
            <div className="flex flex-row p-2 justify-around border-t border-gray-300">
              <button
                onClick={() => handleResume(product)}
                className="text-lg bg-primary p-1 rounded-full text-white shadow-xl shadow-gray-200 hover:shadow-sm"
              >
                Resume
              </button>
              {/* <p className="text-center">${product.offerPrice * product.quantity}</p> */}
              <div className=" content-center">
                <button
                  ...
                  className="cursor-pointer mx-auto flex flex-row"
                >
                  {" "}
                  Remove
