...
{name:"Fruit Tart",img:"https://richanddelish.com/wp-content/uploads/2023/05/mini-fruit-tarts-2.jpg"},
    ...
    {   name:"WelcomeDrinks",
        value: [{name:"Cold Drinks", img:"https://m.media-amazon.com/images/I/71uR7iy29XL._UF350,350_QL80_.jpg"}, 
            {name:"Sweet/ Salty Lemonade", img:"https://i0.wp.com/thejoyfilledkitchen.com/wp-content/uploads/2021/04/Lemonade-2-2.jpg?resize=740%2C792&ssl=1"}, 
            {name:"Strawberry Splash", img:"https://static.toiimg.com/thumb/95210649.cms?imgsize=62398&width=800&height=800"}, 
            {name:"Orange Blossom", img:"https://www.cocktails-road.com/images/recipe/2024/10/orange-blossom-gin-orange-cocktail.jpg"}, 
            {name:"Lime Leechi Squash", img:"https://rachnacooks.com/wp-content/uploads/2023/05/Lycheelemonadefeat2.jpg"}, 
            {name:"Jal Jeera", img:"https://www.whiskaffair.com/wp-content/uploads/2019/03/Jaljeera-2-3.jpg"}, 
            {name:"Pineapple Kick", img:"https://www.eatingwell.com/thmb/tKj5In_qniKaVaAaS3LM7SKdS6k=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/Spicy-Pineapple-Ginger-Mocktail-1x1-1-331eb6839839421ead8dadf2f1891391.jpg"}, 
            {name:"Blue Lagoon", img:"https://img.freepik.com/free-photo/blue-lagoon-martini-glass-garnished-with-orange-slice_140725-548.jpg"}, 
            {name:"Khus Sharbat", img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR6XBhoX5Q7wBckCGPHrFJkFDGJ-shyW82ayA&s"}, 
            {name:"Kokam Sharbat", img:"https://m.media-amazon.com/images/I/61zcbdBuJTL._UF1000,1000_QL80_.jpg"}, 
            {name:"Sole Kadhi", img:"https://www.vegrecipesofindia.com/wp-content/uploads/2012/03/sol-kadhi-recipe-1.jpg"}]
    },   
   ...
   const [rotiCount, setRotiCount] = useState(0);
   
   const countRotis = (newOptions) => {    
     let count =  newOptions?.Rotis?.filter( ( roti )=> Object.values(roti).every(v => v && v !== "null") ).length
     return count
   }
  ...
  setSelectedOptions((prev) => {
    ...
    if (category === "Rotis") {
      const rotis = [...prev.Rotis];

      // Check if already selected
      if (rotis.some((r) => Object.values(r)[0] === itemName)) {
        // Deselect if clicked again
        const newRotis = rotis.map((r) =>
          Object.values(r)[0] === itemName ? { [Object.keys(r)[0]]: null } : r
        );
        updated.Rotis = newRotis;
      } else {
        // Find first empty slot
        const emptyIndex = rotis.findIndex((r) => Object.values(r)[0] === null);

        if (emptyIndex !== -1) {
          rotis[emptyIndex] = { [`item${emptyIndex + 1}`]: itemName };
        } else {
          // Replace first slot if both are filled
          rotis[0] = { item1: itemName };
        }
    ...
    const latestCount = countRotis(updated);
    setRotiCount(latestCount);
    return updated;
  });
...
// }));    
 ...
 useEffect(() => {
   ...
   const productDetail = actualProducts.filter( (product)=> product._id === 'ek51j12k' )
    ...
    <div className='bg-[#EFF6FF]'>
      ...
      { selectedPlatter._id ? ( 
        <div className='grid grid-cols-6 lg:max-w-7xl justify-between mx-auto h-full'>
             ...
             <p className='text-xs text-gray-700'>{rotiCount}/2</p>
             </div>
             <div>
            ...
            <p className='text-xs text-gray-700'>{selectedOptions?.BasmatiRice !==null ? <span>1</span> : <span>0</span>}/1</p>
             ...
             <p className='text-xs text-gray-700'>{selectedOptions?.Soups !==null ? <span>1</span> : <span>0</span>}/1</p>
             </div>
             <div>
             ...
             <p className='text-xs text-gray-700'>{selectedOptions?.Starters !==null ? <span>1</span> : <span>0</span>}/1</p>
             </div>
             <div>
             ...
             <p className='text-xs text-gray-700'>{selectedOptions?.WelcomeDrinks !==null ? <span>1</span> : <span>0</span>}/1</p>
             </div>
             {/* <div>
  ...
  <div>
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1 md:gap-2 p-1 md:p-4">
      {activeSnack?.value.map((snackItem, index) => {
        const isSelected = selectedOptions.Rotis.some(
          (r) => Object.values(r)[0] === snackItem.name
        );
        ...
        return (
          ...
          >
            ...
            <button
              onClick={() => handleSelectedOptions("Rotis", snackItem.name)}
            >
              {isSelected ? (
                <div className="bg-primary text-white mt-3 px-4 py-1 border-2 border-primary rounded-lg">
                  Selected
                </div>
              ) : (
                <div className="mt-3 px-4 py-1 border-2 border-primary text-primary rounded-lg hover:bg-primary hover:text-white">
                  Select
                </div>
              )}
            </button>
...
export default SelectOptionsMenuB