...
const CustomizeBar = (selectedOptions) => {
  const { cart, setCart, selectedPlatter, setSelectedPlatter, navigate } =
    useAppContext();
  const { _id } = useParams();
  // console.log(_id)
  ...
  const handleCart = (_id) => {
    ...
    if (
      Object?.values(selectedPlatter?.selectedOptions)?.filter(
        (item) => item !== null,
      ).length !== 0 &&
      (selectedPlatter.name === "Short Menu" || selectedPlatter.name === "Breakfast" || selectedPlatter.name === "High Tea")
    ) {
      setCart((prev) => {
        const alreadyInCart = prev.some(
          (item) => item._id === selectedPlatter._id,
        );
